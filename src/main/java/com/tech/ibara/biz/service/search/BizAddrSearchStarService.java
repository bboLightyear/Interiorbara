...
@Service
public class BizAddrSearchStarService implements BizServiceInter {

	private SqlSession sqlSession;
	
	public BizAddrSearchStarService(SqlSession sqlSession) {
	...
	public void execute(Model model) {
		...
		System.out.println("service BizAddrSearchStarService>>>");
...
//		시군구가 같은 업체들의 시공리뷰 별점평균 staravg 리스트와 inteno 구해서 bizRvTempStaravgDto에 담기
...
//		JOIN을 통해 시군구가 같은 업체만 골라서 별점순으로 정렬
		ArrayList<BizHomeDto> findHomeLikeInputStar = dao.findHomeLikeInputStar(addrToSearch);
		System.out.println("시군구 입력 후 검색 결과 수: "+findHomeLikeInputStar.size());
        model.addAttribute("bizHomeSearch", findHomeLikeInputStar);
        
		
//		리뷰첫줄 불러오는 함수
		ArrayList<BizRvDto> bizRvContentListJoinStar=dao.bizRvContentListJoinStar(addrToSearch);
		model.addAttribute("bizRvContentListJoinStar", bizRvContentListJoinStar);
		
		ArrayList<String> bizRvContentListStar=new ArrayList<String>();
		
		System.out.println("시군구 입력 후 로딩된 리뷰 첫줄 개수: "+bizRvContentListJoinStar.size());
	
		for (int j = 0; j < bizRvContentListJoinStar.size(); j++) {
			bizRvContentListStar.add(bizRvContentListJoinStar.get(j).getBr_content());
		}
		model.addAttribute("bizRvContentList", bizRvContentListStar);
