...
@Service
public class BizProSearchStarService implements BizServiceInter {

	private SqlSession sqlSession;
	
	public BizProSearchStarService(SqlSession sqlSession) {
	...
	public void execute(Model model) {
		...
		System.out.println("service BizProSearchStarService>>>");
...
//		JOIN을 통해 전문시공영역이 같은 업체만 골라서 별점순으로 정렬
		ArrayList<BizHomeDto> findHomeLikeInputStar = dao.findHomeLikeInputProStar(bh_pro);
		System.out.println("전문시공영역 선택 후 검색 결과 수: "+findHomeLikeInputStar.size());
        model.addAttribute("bizHomeSearch", findHomeLikeInputStar);
        
		
//		리뷰첫줄 불러오는 함수
		ArrayList<BizRvDto> bizRvContentListJoinStar=dao.bizRvContentListJoinProStar(bh_pro);
		...
		System.out.println("전문시공영역 선택 후 로딩된 리뷰 첫줄 개수: "+bizRvContentListJoinStar.size());
